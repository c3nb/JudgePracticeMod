...
namespace JudgePracticeMod
{
    ...
    public static class Main
    {
        ...
        public static void UpdateTV()
        {
            ...
            {
                TagValues["{Accuracy}"] = $"{Math.Truncate((scrController.instance.mistakesManager.percentAcc * 100) * Math.Pow(10, Set.adecimals)) / Math.Pow(10, Set.adecimals)}";
                TagValues["{Progress}"] = $"{Math.Truncate((scrController.instance.currentSeqID / scrController.instance.lm.listFloors.Count - 1) * 100f * Math.Pow(10, Set.pdecimals)) / Math.Pow(10, Set.pdecimals)}";
